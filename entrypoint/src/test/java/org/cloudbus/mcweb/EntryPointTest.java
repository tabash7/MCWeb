...
public class EntryPointTest {
    ...
    public void testConfigureInstanceMultipleTimes() {
        ...
        EntryPoint.configureInstance(
                classLoad(CLOUDSITES_PROPERTIES), 
    ...
    public void testSingleValidUser() {
        ...
        Map<String, Double> latencyCache = ImmutableMap.of(
                req.getIpAddress() + "127.0.0.1", 20d,
                req.getIpAddress() + "127.0.0.2", 50d, 
                req.getIpAddress() + "127.0.0.3", 15d,
                req.getIpAddress() + "127.0.0.4", 40d);
        ...
        Iterable<Set<String>> eligibleUsers = Arrays.asList(new HashSet<String>(Arrays.asList(req.getUserToken())),
                ...
                new HashSet<String>(Arrays.asList(req.getUserToken())),
                new HashSet<String>(Arrays.asList()));
        Iterable<Double> costs = Arrays.asList(20d, 30d, 19d, 50d);
        Function<String[], CloudSite> factory = 
                new PredefinedCostCloudSite.PredefinedCostCloudSiteFactory(eligibleUsers, costs, null);
    ...
    public void testSingleInvalidUser() {
        ...
        Iterable<Set<String>> eligibleUsers = Arrays.asList(new HashSet<String>(Arrays.asList()),
                new HashSet<String>(Arrays.asList()),
                new HashSet<String>(Arrays.asList()),
                new HashSet<String>(Arrays.asList()));
        Iterable<Double> costs = Arrays.asList(20d, 30d, 19d, 50d);
    ...
    private void testMultipleUsersWithDelays(final List<Long> cloudsiteDelays, final long requestDelays) throws InterruptedException {
        ...
        // Cloud Site factory - define eligibilities and costs. Make a user uneleigible everywhere if 
        // i%4 = 0. Eligible in cloud 1 if i%4 = 1. Eligible in clouds 2 and 3 if i%4 = 2. 
        // Eligible in clouds 2 and 4 if i%4 = 3.
        List<Set<String>> eligibleUsers = Arrays.asList(
                new HashSet<String>(),
                new HashSet<String>(),
                new HashSet<String>(),
                new HashSet<String>());
        ...
        Function<String[], CloudSite> factory = 
                new PredefinedCostCloudSite.PredefinedCostCloudSiteFactory(eligibleUsers, costs, cloudsiteDelays);
        ...
        for (int i = 0 ; i < reqs.size(); i++) {
            ...
            if(i % 4 == 0) {
                ...
                if(unresposnive2 && unresposnive4) {
                    ...
                    assertEquals(unresposnive4 || latency2 < latency4? "AWS2" : "AWS4" , req.selectCloudSite().getName());
                } else {
                    assertEquals(unresposnive4 || costs.get(1) < costs.get(3)? "AWS2" : "AWS4" , req.selectCloudSite().getName());
                }
            }
        }
    }

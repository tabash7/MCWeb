...
public class JCloudsManager {
    ...
    public static void main(String[] args) throws RunNodesException, InterruptedException {
        ...
        for (List<String> lst : launchedNodesAddresses) {
    ...
    public static List<List<String>> launchInstances(
        ...
        try {
            ...
            List<String>allowedNames = Arrays.asList(new String[]{"keyPair", "keyPairName"});
            for (Method m : options.getClass().getMethods()) {
                if ( allowedNames.contains(m.getName()) && m.getParameterCount() == 1 && m.getParameterTypes()[0].equals(String.class) ){
                    keyPairMethod = m;
                    break;
                }
            }
            if(keyPairMethod == null) {
                throw new NoSuchMethodException("No key-pair method");
            }
